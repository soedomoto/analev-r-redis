window.eval_file = function() {
  arguments = [].slice.call(arguments);

  var filename = arguments.shift(), 
    callback = null;
  ...
  analev_call('module.file.name.read', [filename], function(_req_id, resp) {
    var resp = JSON.parse(resp);
    if (resp.success) {
      var cmd = resp.data.format(arguments);
      // var cmd = resp.data.format.apply(this, arguments);
...
window.LinearRegressionOLS = class extends BaseModule {
  ...
  process_summarize() {
      eval_file('summary', 
        this.df(), this.state.r_var, this.state.e_vars.join(':'), this.dataset_name(), 
        (data) => {
          this.textarea.value(data.toString());
        });
